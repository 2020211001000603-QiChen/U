...
class PretrainUMCManager:
    ...
    def __init__(self, args, data, model):
        ...
        if args.pretrain:
            ...
            # 如果pretrain=False，不加载预训练模型，使用随机初始化
        ...
        for epoch in trange(int(args.num_pretrain_epochs), desc="Epoch"):
            ...
            for batch in tqdm(pseudo_dataloader, desc = "Iteration"):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    contrastive_logits = torch.cat((norm_mlp_output_a.unsqueeze(1),
                                                    norm_mlp_output_b.unsqueeze(1),
                                                    norm_mlp_output_c.unsqueeze(1)), dim=1)

                    # 兼容配置文件中将 pretrain_temperature 写成单元素列表的情况
                    temperature = args.pretrain_temperature
                    if isinstance(temperature, (list, tuple)):
                        temperature = temperature[0]

                    loss_contrast_mm = self.contrast_criterion(
                        contrastive_logits,
