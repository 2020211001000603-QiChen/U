    ...
    def _get_hyper_parameters(self, args):
        ...
        if args.multimodal_method == 'umc':
            hyper_parameters = {
                ...
                'pretrain_batch_size': 16,        # 进一步减少批次大小以降低内存占用
                'train_batch_size': 16,            # 进一步减少批次大小以降低内存占用
                'eval_batch_size': 8,             # 评估时使用更小的批次
                'test_batch_size': 8,              # 测试时使用更小的批次
                ...
                'train_temperature_sup': [20],      # MELD-DA特有参数
                'train_temperature_unsup': [20],    # MELD-DA特有参数
                'activation': 'tanh',
                'lr_pre': [2e-5],
                'lr': [2e-4],                      # MELD-DA特有参数
                'delta': [0.05],                   # MELD-DA特有参数
                'thres': [0.1],                    # MELD-DA特有参数
                'topk': [5],
                'weight_decay': 0.01,
                'feat_dim': 768,
                'hidden_size': 768,
                ...
                'weight': 1.0,
                'loss_mode': 'rdrop',
                'base_dim': [128],                 # 从256减少到128，减少特征维度
                'nheads': 4,                       # 从8减少到4，降低注意力计算内存占用
                'attn_dropout': 0.1,
                'relu_dropout': 0.1,
                'embed_dropout': 0.1,
                'res_dropout': 0.0,
